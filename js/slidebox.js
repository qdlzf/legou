			...
			$.fn.extend({
				slidebox:function(opts){
					...
					$(this).each(function(index,item){//此处循环是因为选择器可能选择了页面上多个轮播
						...
						$(item).find("#slideMenu .slide").css({"width":"1000px","height":"400px"})

						$(item).find("images").css({"width":boxwidth,"height":boxheight});//修改图片的宽度和高度
						...
						$(item).find(".controls").css({"width":w2+"px","position":"absolute","height":prams.controlsH,"left":left2,"bottom":prams.controltop,"background-color":"transparent" ,"border":"none"});
